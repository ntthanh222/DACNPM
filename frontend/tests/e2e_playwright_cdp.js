...
const SCREENSHOT_DIR = 'd:\\Đồ án CNPM\\testing\\reports\\runtime-first\\corrective-phase\\browser-evidence';
...
async function run() {
    ...
    try {
        ...
        await page.fill('#chatInput', 'Xin chào');
        ...
        await page.fill('#chatInput', 'Kiểm tra URL https://example.com');
        ...
        await page.fill('#chatInput', 'Tra cứu CVE-2024-3094');
        await page.click('button:has-text("send")');
        await page.waitForTimeout(4000);

        // 6 & 7. RAG Runtime & Authenticated SSE
        console.log('--- 6. RAG Runtime ---');
        await page.fill('#chatInput', 'Giải thích các bước harden Ubuntu server');
        await page.click('button:has-text("send")');
        ...
        // Log out and log in as normal user to test forbidden
        await page.evaluate(() => localStorage.clear());
        await page.goto('http://localhost:3000/login.html');
        await page.fill('#input-username', 'testuser@example.com');
        await page.fill('#input-password', 'DevAdmin-2026-T9mQ4vL2!s');
        await page.click('#btn-submit');
        await page.waitForTimeout(3000);
        await page.goto('http://localhost:3000/pages/admin.html');
        await page.waitForTimeout(2000);
        await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'admin-forbidden.png') });

        // 10. Mobile Viewport
        console.log('--- 10. Mobile Viewport ---');
        const mobileCtx = await browser.newContext({ viewport: viewports[2] });
        const mobilePage = await mobileCtx.newPage();
        await mobilePage.goto('http://localhost:3000/pages/assistant/chat.html', { waitUntil: 'domcontentloaded' });
